...
# SSGA 및 API 호출을 위한 User-Agent 설정
HEADERS = {
    ...
    for row in df_meta.values:
        for cell in row:
            if isinstance(cell, str) and "As of" in cell:
                match = re.search(
                    r"As of\s+([0-9]{1,2}-[a-zA-Z]{3}-[0-9]{4})", cell
                )
                if match:
...
def fetch_ssga_aum(etf_symbol, session):
    """SSGA 웹사이트에서 특정 ETF의 NAV 이력 엑셀을 다운로드하여 날짜별 AUM 딕셔너리를 반환합니다.
    안정성을 위해 타임아웃 연장 및 실패 시 최대 3회 재시도를 시도합니다.
    """
    url = f"https://www.ssga.com/us/en/intermediary/library-content/products/fund-data/etfs/us/navhist-us-en-{etf_symbol.lower()}.xlsx"
    max_retries = 3
    timeout = 30  # 대기 시간을 30초로 상향 조정

    for attempt in range(max_retries):
        attempt_suffix = (
            f" (재시도 {attempt + 1}/{max_retries})" if attempt > 0 else ""
        )
        print(f"    [{etf_symbol}] SSGA AUM 데이터 다운로드 중{attempt_suffix}...")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                df_raw = pd.read_excel(
                    io.BytesIO(response.content), header=None
                )

                header_row_idx = None
                for idx, row in df_raw.iterrows():
                    row_vals = [
                        str(x).strip().lower() for x in row if pd.notna(x)
                    ]
                    if any("date" in val for val in row_vals) and any(
                        "net assets" in val for val in row_vals
                    ):
                        header_row_idx = idx
                        break
                ...
                if header_row_idx is not None:
                    df = pd.read_excel(
                        io.BytesIO(response.content), skiprows=header_row_idx
                    )
                    ...
                    for col in df.columns:
                        ...
                        if "date" in col_lower:
                            date_col = col
                        elif "net assets" in col_lower or "aum" in col_lower:
                            aum_col = col

                    if date_col and aum_col:
                        # 날짜 형식 표준화
                        ...
                        return dict(zip(df["date_str"], df[aum_col]))

                # 올바른 헤더 형식이 아닐 경우 재시도 없이 루프 탈출
                print(
                    f"    [{etf_symbol}] 엑셀 내 유효한 헤더를 찾지 못했습니다."
                )
                break

            else:
                print(
                    f"    [{etf_symbol}] 다운로드 실패 (HTTP 상태 코드: {response.status_code})"
                )
                if attempt < max_retries - 1:
                    time.sleep(2)

        except (requests.exceptions.RequestException, Exception) as e:
            # 타임아웃 및 네트워크 끊김 현상 감지
            print(f"    [{etf_symbol}] 연결 실패 또는 타임아웃 발생: {e}")
            if attempt < max_retries - 1:
                print(f"    [{etf_symbol}] 2초 후 재시도합니다...")
                time.sleep(2)
            else:
                print(f"    [{etf_symbol}] 최대 재시도 제한을 초과했습니다.")

    return {}


def main():
    print("🚀 ETF 수급 분석 시작 (공식 SSGA AUM 정보 매칭)...\n")
    ...
    # requests.Session을 사용하여 커넥션을 효율적으로 유지
    with requests.Session() as session:
        for etf in ETF_LIST:
            ...
            try:
                response = session.get(excel_url, headers=HEADERS)
            ...
            if excel_date_str in metrics_data.get(etf, {}).get("history", {}):
                print(
                    f"    * {etf}: 이미 오늘자({excel_date_str}) 데이터가 있습니다.\n"
                )
                continue
            ...
            df_holdings = df_holdings[
                df_holdings["Ticker"].apply(is_valid_ticker)
            ]
            ...
            end_date_yf = (excel_date_obj + timedelta(days=1)).strftime(
                "%Y-%m-%d"
            )
            start_date_yf = (excel_date_obj - timedelta(days=45)).strftime(
                "%Y-%m-%d"
            )
            ...
            etf_history = yf.download(
                etf, start=start_date_yf, end=end_date_yf, progress=False
            )
            ...
            # 4. SSGA 웹사이트에서 실제 AUM 수집 및 매칭
            aum_val = None
            try:
                aum_map = fetch_ssga_aum(etf, session)
                excel_aum = aum_map.get(excel_date_str)

                if excel_aum is not None and pd.notna(excel_aum):
                    aum_val = get_raw_aum(excel_aum)
                else:
                    # 매칭 실패 시, 기존 json의 동일 일자 백업 데이터 확인
                    existing_etf_price = (
                        metrics_data.get(etf, {})
                        .get("history", {})
                        .get(excel_date_str, {})
                        .get("ETF_Price", {})
                    )
                    existing_net_assets = existing_etf_price.get(
                        "net_assets"
                    ) or existing_etf_price.get("AUM")

                    if existing_net_assets is not None:
                        if isinstance(existing_net_assets, dict):
                            raw_val = existing_net_assets.get(
                                "value"
                            ) or existing_net_assets.get("display")
                            aum_val = (
                                get_raw_aum(raw_val)
                            ...
                            aum_val = get_raw_aum(existing_net_assets)
            except Exception as e:
                print(f"    * {etf} AUM 정보 수집 실패: {e}")
            ...
            try:
                ...
                if len(spy_close_arr) >= 6:
                    spy_return_5d = (
                        (float(spy_close_arr[-1]) - float(spy_close_arr[-6]))
                        / float(spy_close_arr[-6])
                    ) * 100
            except Exception as e:
            ...
            avg_vol_20 = (
                float(etf_vol_arr[-21:-1].mean())
                if len(etf_vol_arr) > 20
                else 0.0
            )
            ...
            try:
                excess_return = (
                    ((etf_price - float(etf_close_arr[-6])) / float(etf_close_arr[-6]))
                    * 100
                ) - spy_return_5d
            except:
                excess_return = 0.0
            ...
            saved_detail_files = sorted(
                [f for f in os.listdir(etf_dir) if f.endswith(".json")]
            )
            ...
            for ticker, c_data in current_raw_data.items():
                ...
                share_diff = (
                    c_shares
                    - last_raw_data.get(ticker, {}).get("shares", c_shares)
                    if not is_first_run
                    else 0
                )
                ...
                if (
                    isinstance(stocks_data, pd.DataFrame)
                    and ticker in stocks_data.columns
                ):
                    valid_series = stocks_data[ticker].dropna()
                    ...
                    if len(prices_arr) >= 2 and float(prices_arr[-2]) > 0:
                        price_change_pct = (
                            (price - float(prices_arr[-2]))
                            / float(prices_arr[-2])
                        ) * 100

                    if len(prices_arr) >= 6 and float(prices_arr[-6]) > 0:
                        stock_return_5d = (
                            (price - float(prices_arr[-6]))
                            / float(prices_arr[-6])
                        ) * 100
                ...
                flow_ratio_pct = (
                    (share_diff / (abs(share_diff) + c_shares) * 100)
                    if holdings_value > 0
                    else 0.0
                )
            ...
            fund_flow_display = (
                "데이터 누적 중"
                if is_first_run
                else format_flow(total_fund_flow_dollar)
            )
            flow_ratio_20d_display, flow_ratio_20d_value = "데이터 부족", 0.0

            if etf not in metrics_data:
                metrics_data[etf] = {"history": {}}
            past_dates = sorted(
                list(metrics_data[etf]["history"].keys())
            )

            if len(past_dates) >= 20:
                recent_20 = past_dates[-20:]
                past_flows_abs = [
                    abs(metrics_data[etf]["history"][d]["Fund_Flow"]["value"])
                    for d in recent_20
                ]
                avg_flow_20d = (
                    sum(past_flows_abs) / len(past_flows_abs)
                    if len(past_flows_abs) > 0
                    else 0
                )

                if avg_flow_20d > 0:
                    flow_ratio_20d_value = (
                        abs(total_fund_flow_dollar) / avg_flow_20d
                    )
            ...
            metrics_data[etf]["history"][excel_date_str] = {
                ...
                "Fund_Flow": {
                    "value": round(total_fund_flow_dollar, 2),
                    "display": fund_flow_display,
                },
                "Fund_Flow_Intensity_20D": {
                    "value": round(flow_ratio_20d_value, 2),
                    "display": flow_ratio_20d_display,
                },
                "Relative_Volume": {
                    "value": round(rvol, 2),
                    "display": f"{rvol:.1f}배",
                ...
                "RS_Momentum": {
                    "value": round(excess_return, 2),
                    "display": f"SPY 대비 {'+' if excess_return > 0 else ''}{excess_return:.2f}%p",
                },
                "Weighted_Breadth": {
                    "value": round(weighted_breadth, 2),
                    "display": f"{weighted_breadth:.1f}%",
                },
            }
            ...
            print(
                f"    [성공] 지표 업데이트 완료 & 상세파일 생성 ({detail_file_path})\n"
            )
